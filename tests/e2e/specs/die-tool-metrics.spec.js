...
test.describe('Die tool metrics and counter', () => {
  ...
  test('@smoke submitted manufacture entry populates duration SPM cycle and efficiency', async ({ page }) => {
    await page.goto('/app/home');
    const ctx = await setupFreshContext(page, lifecycle.getPrefix());

    const stockEntryPage = await createManufactureEntry(page, ctx, { fgQty: 100, rejectionQty: 5 });
  ...
  test('@regression high utilization sets maintenance due and shows warning headline', async ({ page }) => {
    await page.goto('/app/home');
    const ctx = await setupFreshContext(page, lifecycle.getPrefix());

    await callFrappeMethod(page, 'production_entry_app.production_entry_app.api.get_die_tool_counter', {
    ...
    const stockEntryPage = await createManufactureEntry(page, ctx, { fgQty: 10, rejectionQty: 0 });
    await stockEntryPage.saveDraft();
  ...
  test('@regression die-tool counter increases on submit and decreases on cancel', async ({ page }) => {
    await page.goto('/app/home');
    const ctx = await setupFreshContext(page, lifecycle.getPrefix());
    ...
    const stockEntryPage = await createManufactureEntry(page, ctx, { fgQty: 100, rejectionQty: 10 });
    ...
    expect(Number(afterSubmit.current_strokes)).toBeGreaterThan(Number(before.current_strokes));
