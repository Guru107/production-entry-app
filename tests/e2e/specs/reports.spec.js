...
async function createSubmittedStockEntryForReports(page, ctx, rejectionQty = 0) {
  ...
  if (rejectionQty > 0) {
    await stockEntryPage.setRejectionBreakupRows([{ rejection_reason: 'Burr', qty: rejectionQty }]);
  }
...
test.describe('Production reports', () => {
  ...
  test('@regression Workstation report honors workstation and shift filters', async ({ page }) => {
    await page.goto('/app/home');
    const prefix = lifecycle.getPrefix();
    const ctx = await setupFreshContext(page, prefix);
    const seeded = await createSubmittedStockEntryForReports(page, ctx, 0);
  ...
  test('@regression Die Tool report honors item filter and shows maintenance columns', async ({ page }) => {
    await page.goto('/app/home');
    const prefix = lifecycle.getPrefix();
    const ctx = await setupFreshContext(page, prefix);
