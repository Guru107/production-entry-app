...
test.describe('Stock Entry validation matrix', () => {
  ...
  test('@smoke custom_fetch_items requires qty before fetching', async ({ page }) => {
    ...
    const stockEntryPage = await openManufactureEntry(page, ctx, { fgQty: 0, rejectionQty: 0 });
  ...
  test('@regression rejection qty with empty breakup blocks save', async ({ page }) => {
    ...
    const stockEntryPage = await openManufactureEntry(page, ctx, { fgQty: 100, rejectionQty: 5 });
    await stockEntryPage.fetchItems();
  ...
  test('@regression rejection qty greater than finished good qty blocks save', async ({ page }) => {
    await page.goto('/app/home');
    const ctx = await setupFreshContext(page, lifecycle.getPrefix());

    const stockEntryPage = await openManufactureEntry(page, ctx, { fgQty: 100, rejectionQty: 150 });
    await stockEntryPage.fetchItems();
  ...
  test('@regression actual start outside configured buffer blocks save with range message', async ({ page }) => {
    await page.goto('/app/home');
    const ctx = await setupFreshContext(page, lifecycle.getPrefix());
  ...
  test('@regression unplanned loss row can be added and persists after save', async ({ page }) => {
    await page.goto('/app/home');
    const ctx = await setupFreshContext(page, lifecycle.getPrefix());

    const stockEntryPage = await openManufactureEntry(page, ctx, { fgQty: 100, rejectionQty: 0 });
    await stockEntryPage.fetchItems();
  ...
  test('@regression re-save remains idempotent for rejection row and finished good qty', async ({ page }) => {
    await page.goto('/app/home');
    const ctx = await setupFreshContext(page, lifecycle.getPrefix());

    const stockEntryPage = await openManufactureEntry(page, ctx, { fgQty: 100, rejectionQty: 10 });
    await stockEntryPage.fetchItems();
    await stockEntryPage.setRejectionBreakupRows([
    ...
    const savedStockEntry = await getDoc(page, 'Stock Entry', stockEntryName);
    const rejectionRows = (savedStockEntry.items || []).filter((row) => Boolean(row.custom_is_rejection_item));
    const fgRows = (savedStockEntry.items || []).filter((row) => Boolean(row.is_finished_item));
