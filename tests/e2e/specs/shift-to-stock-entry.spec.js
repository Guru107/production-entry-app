...
test.describe('Shift to Stock Entry integration', () => {
  ...
  test('@smoke running shift create action opens stock entry with shift prefilled', async ({ page }) => {
  ...
  test('@regression selecting shift auto-fills branch planned dates and warehouses', async ({ page }) => {
    await page.goto('/app/home');
    const ctx = await setupFreshContext(page, lifecycle.getPrefix());
    ...
    expect(String(values.custom_planned_start_date || '')).toContain(`${ctx.shift_date} 08:00:00`);
  ...
  test('@regression clearing shift clears auto-filled planning and warehouse fields', async ({ page }) => {
    await page.goto('/app/home');
    const ctx = await setupFreshContext(page, lifecycle.getPrefix());
