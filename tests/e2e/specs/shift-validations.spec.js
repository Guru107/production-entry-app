...
test.describe('Shift validations', () => {
  ...
  test('@regression planned losses auto-populate and repopulate on duration change', async ({ page }) => {
    await page.goto('/app/home');
    const ctx = await bootstrapE2E(page, lifecycle.getPrefix());
    const shiftPage = new ShiftPage(page);
  ...
  test('@regression linked downtime section renders overlapping downtime entries', async ({ page }) => {
    await page.goto('/app/home');
    const ctx = await bootstrapE2E(page, lifecycle.getPrefix());
    const shiftPage = new ShiftPage(page);
    ...
    try {
      ...
      await shiftPage.open(ctx.shift_name);
      const linkedSection = page.locator('[data-fieldname="linked_downtime_entries"]').first();
