...
class StockEntryPage {
  ...
  async openNew() {
    await this.page.goto('/app/stock-entry/new');
  ...
  async open(name) {
    ...
    await this.page.goto(`/app/stock-entry/${encodedName}`);
    await expect(this.page).toHaveURL(new RegExp(`/app/stock-entry/${encodedName}$`));
    ...
    await this.page.waitForFunction(
      ({ expectedBranch, expectedWarehouse, startSnippet, endSnippet }) => {
        ...
        const warehouseMatch =
          expectedWarehouse
            ? doc.from_warehouse === expectedWarehouse && doc.to_warehouse === expectedWarehouse
            : true;
    ...
    return await this.page.evaluate(async (searchText) => {
      ...
      return await new Promise((resolve, reject) => {
        frappe.call({
          ...
          error: (err) => reject(new Error(err?.message || 'Failed to search Shift link options.')),
        });
      });
    }, text);
  }
  ...
  async saveAndSubmit() {
    ...
    const doc = await callFrappeMethod(this.page, 'frappe.client.get', { doctype: 'Stock Entry', name });
