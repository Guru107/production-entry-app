...
class ReportsPage {
  ...
  async open(reportName) {
    ...
    await this.page.waitForFunction(
      (name) => Boolean(window.frappe?.query_report) && window.frappe.query_report.report_name === name,
      reportName
    );
    await this.page.waitForFunction(() => {
      const report = window.frappe?.query_report;
      return Boolean(report && (Array.isArray(report.filters) || Array.isArray(report.report_settings?.filters)));
    });
  }
  ...
  async setFilterByFieldname(fieldname, value) {
    await this.page.evaluate(
      async ({ key, val }) => {
        ...
        const filter = typeof report.get_filter === 'function' ? report.get_filter(key) : null;
        if (!filter) {
  ...
  async setFilterByLabel(label, value) {
    await this.page.evaluate(
      async ({ filterLabel, val }) => {
        ...
        const filter = (report.filters || []).find((row) => row?.df?.label === filterLabel);
        if (!filter) {
