...
shrink: E2E-only setup and cleanup helpers exposed from production `api.py`. Keep the Playwright bootstrap/cleanup contract, move scenario fixture work out of the main API module, and delete unit tests for helpers that stop being product API. Trade-off: Playwright keeps fewer remote knobs, but the production API surface and `test_api.py` get much smaller. [production_entry_app/production_entry_app/api.py, production_entry_app/production_entry_app/test_api.py, tests/e2e]
