...
- `operator`: group key (`custom_operator`, fallback `"Unassigned"`).
- `entries`: count of Stock Entries in group.
- `good_qty`: `total_strokes - rejection_qty` aggregated per group.
...
- `standard_spm`: `standard_units / duration_mins` where `standard_units = Σ(custom_standard_spm * duration_mins)`.
