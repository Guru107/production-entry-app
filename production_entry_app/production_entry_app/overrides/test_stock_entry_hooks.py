...
class TestStockEntryHooks(FrappeTestCase):
	...
	def test_rejection_hook_runs_after_submit_modification(self) -> None:
		"""Verify allow_on_submit: 1 allows post-submit modification of rejection fields.

		ERPNext v16's "Update Items" workflow modifies custom_rejection_qty on submitted
		Stock Entries. With allow_on_submit: 1, the validate hook must still rebuild
		rejection rows correctly on save.
		"""
		_ensure_rejection_breakup_doctype()
		_ensure_rejection_reason_doctype()
		_ensure_rejection_reasons()
		...
		# Reload from DB (simulates post-submit edit via "Update Items")
		se.reload()
		se.custom_rejection_qty = 8
		# Update breakup to match new total
		se.custom_rejection_breakup = []
		_append_rejection_breakup_rows(
			se,
			[
				{"rejection_reason": "Burr", "qty": 5, "is_rework": 1},
				{"rejection_reason": "Crack", "qty": 3, "is_rework": 0},
			],
		)
		# save() triggers validate which calls _apply_rejection_entries
		se.save()
		self.assertEqual(se.custom_rejection_qty, 8)
		self.assertEqual(se.custom_rework_qty, 5)  # only Burr is rework
