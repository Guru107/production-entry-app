...
def _get_rows(filters: dict) -> list[dict]:
	...
	for entries in iter_stock_entries_in_chunks(
		...
		for entry in entries:
			...
			if entry_name:
				agg["setting_time_hrs"] += flt((loss_metrics.get("setup_mins") or 0) / 60)
				agg["loss_time_hrs"] += flt((loss_metrics.get("loss_mins") or 0) / 60)
			...
			agg["total_strokes"] += flt(total_strokes)
	...
	for key in sorted(aggregates.keys()):
		...
		production_mins = flt(agg["production_mins"])
		setting_time_hrs = flt(agg["setting_time_hrs"])
		loss_time_hrs = flt(agg["loss_time_hrs"])
		production_time_hrs = flt(production_mins / 60) if production_mins > 0 else 0
		spm = flt((agg["total_strokes"] / (production_time_hrs * 60))) if production_time_hrs > 0 else 0
		rows.append(
			{
				"date": agg["date"],
