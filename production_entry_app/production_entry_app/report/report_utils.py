	...
	for row in loss_rows:
		...
		if row.get("downtime_reason") == SETUP_TIME_REASON:
			parent_metrics[parent]["setup_mins"] = flt(
				parent_metrics[parent]["setup_mins"] + duration_mins)
		else:
			parent_metrics[parent]["loss_mins"] = flt(parent_metrics[parent]["loss_mins"] + duration_mins)
	return parent_metrics
...
def get_entry_production_minutes(
	...
	return flt(max(duration_mins - flt(setup_mins, 3) - flt(loss_mins, 3), 0))
...
def accumulate_efficiency_aggregate(
	...
	total_units = flt(good_qty + rejection_qty)
...
def build_efficiency_rows(
	...
	for group_value, agg in sorted(aggregates.items()):
		...
		actual_spm = flt((agg["total_units"] / duration_mins)) if duration_mins > 0 else 0
		standard_spm = flt(agg["standard_spm"])
		if duration_mins <= 0 and entry_count:
			actual_spm = flt((agg["actual_spm_sum"] / entry_count))
		efficiency_pct = flt((actual_spm / standard_spm) * 100) if standard_spm > 0 else 0
		rows.append(
			{
