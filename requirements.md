...
- On setting the shift reference field, it must automatically fetch "branch", set "custom_planned_start_date" and "custom_planned_start_date" from the shift document. Assume that these fields are already available in the stock entry document.
- There are fields called "custom_planned_start_date" and "custom_planned_end_date" in the stock entry doctype, those must be pre-filled with shift start date time and end date time as these are DateTime fields.
...
- "Operation Details" will have the following fields,
  - "custom_planned_start_date" (Shift Planned start date time)
  - "custom_planned_start_date" (Shift Planned end date time)
  - "custom_actual_start_date" (Supervisor enters)
  - "custom_actual_end_date" (Supervisor enters)
  - "custom_workstation" (Link)
  - "custom_standard_spm" (Fetch from Workstation)
